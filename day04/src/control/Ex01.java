...
public class Ex01 {
	...
	public static void main(String[] args) {
		...
		// 3) 기타 : 반복흐름을 강제로 끊거나. 함수진행을 강제로 종료한다(beak, continue, return)

		// 분기문과 반복문은 특정 조건에 의해서 수행되므로
		// 비교연산, 논리연산 등을 활용하여 boolean 타입의 값을 타루는 연습이 필요하다
		// boolean을 이용하여 새로운 boolean을 만들어낸다

		// 사용자의 "입력 값이 무엇이냐에 따라, 어떤 크기의 값이냐에 따라
		// 조건을 생성하고 조건에 따라 서로 다른 코드를 수행하게 한다
		// 분기문뿐만아니ㄹ, 반복문도 boolean 타입의 값을 사용하기 때문에
		...
		while (true) { // 무한 반복(조건이 항상 참이기 때문에, 계속 반복한다
